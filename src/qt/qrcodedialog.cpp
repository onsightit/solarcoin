...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SLR unit input in lnReqAmount, we generate the URI with SLR as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::formatMaxDecimals(BitcoinUnits::SLR, ui->lnReqAmount->value(), BitcoinUnits::maxdecimals(BitcoinUnits::SLR)));
            paramCount++;
        }
        else
        {
