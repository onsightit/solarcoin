...
GetSolarCoinPage::GetSolarCoinPage(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::GetSolarCoinPage),
    walletModel(0)
{
    ui->setupUi(this);
    ...
    if (fNoHeaders)
        ...
        GUIUtil::header(this, QString(":images/headerGetSolarCoinSmall"));
    else
        GUIUtil::header(this, QString(":images/headerGetSolarCoin"));
    this->layout()->setContentsMargins(0, HEADER_HEIGHT, 0, 0);

    CookieJar *getSolarCoinJar = new CookieJar;
    ui->webView->page()->networkAccessManager()->setCookieJar(getSolarCoinJar);
...
void GetSolarCoinPage::setModel(WalletModel *model)
{
