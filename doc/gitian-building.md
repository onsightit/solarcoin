...
Clone the git repositories for solarcoin and Gitian.
...
in the solarcoin repository under 'Fetch and create inputs' to install sources which require
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/solarcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/solarcoin-project/solarcoin
...
For example:
```bash
URL=https://github.com/thrasher-/solarcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit solarcoin=${COMMIT} --url solarcoin=${URL} ../solarcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit solarcoin=${COMMIT} --url solarcoin=${URL} ../solarcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit solarcoin=${COMMIT} --url solarcoin=${URL} ../solarcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the solarcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../solarcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/solarcoin
SIGPATH=/some/root/path/solarcoin-detached-sigs

./bin/gbuild --url solarcoin=${BTCPATH},signature=${SIGPATH} ../solarcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/solarcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/solarcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/solarcoin-osx-build.assert
```
...
[solarcoin-project/gitian.sigs.slr](https://github.com/solarcoin-project/gitian.sigs.slr/) repository, or if that's not possible create a pull
